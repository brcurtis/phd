...
function [ Af ] = affinity( Ab, Ag )
    ...
    Af = 1 ./ (1 + sqrt(sum((x(:,1:Ab_size(2)-1) - Ab(:,1:Ab_size(2)-1)).^2,2)));
end
...
function [ eval ] = ais_eval( Ag_eval, Ag_eval_size, Abm )
    ...
    for i = 1:Ag_eval_size(1)
        ...
        Abn                 = select(Abm, Af, k);                   % Step 2: Select top antibody (Abm)
        %fprintf('Ag_eval = %d \t Af = %f \t Abn = %d \t',Ag_eval(i,6),Abn(7),Abn(6));
        temp = 0;
        for j = 1:k
            if Ag_eval(i,6) == Abn(j,6)
                temp = temp + 1;
            end
            %fprintf('success = %d \n',success);
        end
...
%     for i = 1:Ag_eval_size(1)
%         Af                  = affinity(Abm, Ag_eval(i,:));          % Step 1: Measure affinity (Af) of the antigen (Ag_val(i,:)) against each antibody (Abm)
%         Abn                 = select(Abm, Af, 1);                   % Step 2: Select top antibody (Abm)
%         %fprintf('Ag_eval = %d \t Af = %f \t Abn = %d \t',Ag_eval(i,6),Abn(7),Abn(6));
%         if Ag_eval(i,6) == Abn(6)
%             success = success + 1;
%         end
%         %fprintf('success = %d \n',success);
%     end
